...
from ..forms import AdminSignUpForm,TimetableCreateForm,RequestNoteForm
from ..models import *
...
def RequestView(request):
    requests = Request.objects.filter(school=request.user.school)
    # requests = Request.objects.filter(school=request.user.school,noted=False)
    # RequestFormset = formset_factory(RequestNoteForm,extra=requests.count())

    # if request.method == 'POST':
    #     formset = RequestFormset(request.POST)
    #     if formset.is_valid():
    #         for i,form in enumerate(formset):
    #             form.save(requests[i].id)
    #         return redirect('admin:requests')
    # else:
    #     formset = RequestFormset()
    # a = zip(requests,formset)
    # print(['*']*50,a,requests.count())
    return render(request, 'classroom/admin/requests_list.html', {
        'requests' : requests
    })